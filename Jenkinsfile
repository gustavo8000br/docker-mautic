  ...
  stages {
    ...
    stage("Set ENV github_stable"){
     steps{
       script{
         env.EXT_RELEASE = sh(
           script: '''curl -s https://api.github.com/repos/${EXT_USER}/${EXT_REPO}/releases | jq -r 'first(.[] | select(.prerelease == true)) | .tag_name' ''', 
           returnStdout: true).trim()
       }
     }
    }
    ...
    // If this is a beta-apache build use live docker endpoints
    ...
    stage('Github-Tag-Push-Release') {
      ...
      steps {
        ...
        -d '{"tag":"'${EXT_RELEASE_CLEAN}'-'${EXT_VERSION_TYPE}'-build-'${MY_TAG_NUMBER}'",\
             "object": "'${COMMIT_SHA}'",\
             "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-'${EXT_VERSION_TYPE}'-build-'${MY_TAG_NUMBER}' to beta-apache",\
             "type": "commit",\
